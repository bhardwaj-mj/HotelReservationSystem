...
//    public Map<Integer, Hotel> getCheapestBestRatedHotels(int weekDays, int weekendDays)
//    {
//        Map<Integer, Hotel> hotelRating = new HashMap<>();
//        Map<Integer, Hotel> sortedHotelCosts = new HashMap<>();
//        if (hotels.size() == 0)
//            return null;
//        this.hotels.stream().forEach(
//                n -> hotelRating.put(n.getRating(), n));
//        Integer best = hotelCosts.keySet().stream().min(Integer::compare).get();
//        hotelCosts.forEach((k, v) -> {
//            if (k == cheap)
//                sortedHotelCosts.put(k, v);
//        });
